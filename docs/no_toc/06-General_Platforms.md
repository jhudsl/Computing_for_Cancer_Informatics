...
## Multiple Experiment Viewer (WebMeV)

[WebMeV](https://webmev.tm4.org/#/about) is a free and open-source cloud-based web application with a graphical interface for analyzing genomic data. It is particularly designed for processing raw sequencing data and can now support larger-scale levels of data. The tools available mostly focus on RNA sequencing data, including differential expression analysis and gene set enrichment analysis. There is now also access to data from [The Cancer Genome Atlas (TCGA)](https://en.wikipedia.org/wiki/The_Cancer_Genome_Atlas) and the  [Therapeutically Applicable Research to Generate Effective Treatments (TARGET) project](https://ocg.cancer.gov/programs/target/data-matrix). See [here](https://web-mev.github.io/) for a guide on how to use the WebMeV application including a [case study](https://web-mev.github.io/rnaseq_case_study/).

## Overture

Overture is a relatively new option for perform large-scale genomic data analyses. You can upload, download, manage, analyze and share your data with authentication and authorization methods to add security. Although designed for genomic research, the [data management system](https://www.overture.bio/documentation/dms/) can be used for other scientific domains. Currently, additional products are still being developed for analysis, visualization, and sharing. However, several collaborations have created new incredible resources using some of the existing and developing products that might be useful for your research. Alternatively, Overture has options to help you create your own platform, see [here](https://www.overture.bio/services/) for more information. 
